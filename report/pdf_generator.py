...
def generate_pdf_report(results, output_dir='reports'):
    ...
    filename = f"SiteShield_Report_{domain}_{timestamp}.pdf"
    ...
    title_style = ParagraphStyle(
        ...
        leading=42  # ADD THIS - controls line height
    )
    ...
    story.append(Spacer(1, 1.5*inch))
    story.append(Paragraph("Website Security Report", title_style))
    story.append(Spacer(1, 0.3*inch))  # ADD THIS LINE
    story.append(Paragraph(f"<b>{domain}</b>", subtitle_style))
    ...
    brand_text = '<font size="12"><b>SiteShield</b> - Professional Website Security Scanner</font><br/><font size="10">https://siteshield.io</font>'
    ...
    for i, rec in enumerate(recommendations, 1):
        ...
        # Impact
        impact_html = rec['impact'].replace('\n', '<br/>')
        story.append(Paragraph(f"<b>Impact:</b><br/>{impact_html}", body_style))
        ...
        # conver new lines to html breaks for proper display
        fix_html = rec['fix'].replace('\n\n', '<br/><br/>').replace('\n', '<br/>')

        # Bold Section Headers
        fix_html = fix_html.replace('WHY IT MATTERS:', '<b>WHY IT MATTERS:</b>').replace('POTENTIAL IMPACTS:', '<b>POTENTIAL IMPACTS:</b>').replace('FIX STEPS:', '<b>FIX STEPS:</b>')

        # Bold step numbers
        import re
        fix_html = re.sub(r'(Step \d+:)', r'<b>\1</b>', fix_html)

        # Italicize platform headers
        fix_html = re.sub(r'^(For [^:]+:)', r'<br/><i>\1</i>:', fix_html)
        ...
        story.append(Paragraph(fix_html, fix_style))
        story.append(Spacer(1, 0.3*inch))
    ...
    # Footer
    footer = """
    <i>This report was generated by SiteShield, an automated security scanner. 
...
def generate_recommendations(results):
    """Generate prioritized recommendations with AI-powered fixes"""
    from scanner.fix_generator import generate_port_fix, generate_header_fix
    
    recommendations = []
    domain = results.get('domain', 'your-website.com')
    
    # SSL recommendations (keep existing - it's good)
    if not results['ssl'].get('has_ssl'):
...
For Nginx:
    ...
    for port_info in risky_ports:
        ...
        recommendations.append({
            ...
            'issue': fix_data['why'],
            'impact': '\n'.join(f"• {impact}" for impact in fix_data.get('impacts', [])),
            'fix': fix_data['raw_fix'],
            'ai_generated': fix_data.get('ai_generated', False)
        })
    
    # AI-POWERED HEADER FIXES (NEW!)
    critical_headers = [h for h in results['headers'].get('missing_headers', []) if h['risk'] == 'High']
    for header in critical_headers[:2]:  # Top 2 only
        fix_data = generate_header_fix(
...
3. Verify DNS propagation globally
    ...
    if results['dns'].get('mx_records') and not results['dns'].get('has_spf'):
        recommendations.append({
            ...
            'impact': 'Attackers can send emails pretending to be from your domain.',
            'fix': """
...
- Microsoft 365: v=spf1 include:spf.protection.outlook.com ~all
    ...
    # Certificate expiration (keep existing)
    ...
    if days_left and days_left < 30:
        recommendations.append({
            ...
            'impact': 'When certificate expires, browsers will show warnings to all visitors.',
            'fix': """
